...
THE GENERAL DISTRIBUTION  KNOWN VARIANCE CASE}
...
Indifference Zone (BIZ) procedure, proposed in \citeN{Frazier:BIZ}, is
...
% In such ranking and selection problems, we must decide how many samples we need to accurately find the alternative with the largest mean. 
...
More recently, new algorithms were developed in \citeN{Kim:2001}, \citeN{Goldsman:2002}, 
...
\citeN{Frazier:BIZ} eliminates the use of Bonferroni's inequality, reducing conservatism. This paper presents a new sequential elimination
...
\citeN{kim:asympt} also proves the asymptotical validity of a IZ procedure in the same limit.
Our proof shares some similarities since we both use a central limit theorem.

Two other common sources of conservatism are the change from discrete-time processes to continuous-time processes used to show IZ guarantees, and the fact that typically, the configuration under consideration is not a worst-case configuration \cite{Kim:Conserv}.  
This last source tends to be the largest in terms of its contribution to conservatism, and the first tends to be the smallest, with the contribution from Bonferonni inequality's lying between \cite{Kim:Conserv}.
Although the BIZ procedure avoids the Bonferonni inequality, it does not avoid these other two sources of conservatism.
Indeed, all indifference zone procedures must meet the PCS guarantee for all configurations in the preference zone, including worst-case configurations.  In this sense, this last source of conservatism arises from the indifference zone guarantee itself, rather than the from a design of a particular procedure.
...
This algorithm generalizes the BIZ procedure with known common variance. In that case, we have that$B_1=\cdots=B_k=1$ and $n_{tx}=t$. The algorithm 2 can be generalized to the continuous case (See  \citeN{Frazier:BIZ}). 
...
% Thus, we can see the selection decision resulting from BIZ as being closely app
...
% Observe that $\mathcal{C}\left(\delta,\cdot\right)\in D\left[0,\infty\right)^{k}$ and the convergence of Lemma 1 is in $D\left[0,\infty\right)^{k}$, consequently the following natural step is to generalize the continuous-time procedure for the heterogeneous variance setting, see Appendix B, to the space $D\left[0,\infty\right)^{k}$. 
...
% In this sense, 
% we consider to be $q_{tx}^{F,\delta}\left(A\right)$ a ``non-centralized'' version of $F$.
...
% to a problem with indifference-zone parameter equal to 1, where each alternative's observation process has volatility $\lambda_z$ and drift $a_x$.
...
% Going back to the proof outline, 
% the next step is to find the limit of $q_{tx}^{F,\delta}\left(A\right)$
% when $\delta$ goes to zero, which we denote $q_{tx}^{F}\left(A\right)$, 
% and is equal to
\[
...
% we can prove that the difference of the probability of correct selection of the discrete-time BIZ algorithm and the  probability of correct selection of the continuous-time BIZ algorithm converge to zero as $\delta$ goes to zero. Then we can use the continuous-time BIZ algorithm in the limit, i.e. we can use $\mathcal{C}\left(\delta,\tau_{n}\left(\delta\right)\right)$ instead
% of $\mathcal{C}\left(\delta,\hat{\tau}_{n}\left(\delta\right)\right)$. The details may be found in the full version of this paper \cite{fullpaper}. 
The following lemma allows us to use the continuous-time BIZ algorithm in the limit. A proof Lemma~\ref{l:interpolation} may be found in a full version of this paper \cite{fullpaper}.

\begin{lemma}
If samples from system $x\in\left\{ 1\ldots,k\right\} $ are identically
distributed with finite variance and independent, over time and across alternatives, then
\begin{equation*}
...
%we can work with the algorithm, which we call the discrete-time BIZ algorithm, defined by
...
%\end{eqnarray*}
% The corresponding BIZ algorithm is called the continuous-time BIZ algorithm. We denote the corresponding continuous hitting times by $\left(\tau_{n}\left(\delta\right)\right)_{n}$,
% which are defined as 


% Maybe some of this goes into the proof of the new lemma
\[
...
% and allows us to use Theorem~5.5 of \citeN{billingsley:convergence}.
...
\end{equation*}
By Lemma~\ref{l:interpolation}, as $\delta\to0$, we have that 
\begin{equation}
\lim_{\delta\to0}
\mbox{PCS}(\delta)
...
($c=1-(P^*)^\frac{1}{k-1}$) when $\delta$ goes to zero. We plot the PCS of three different examples.
...
BIZ procedure. Here, the points plotted have central confidence intervals of length at most $0.014$. 
...
\begin{figure}[h]
  \centering
   ...
   \end{subfigure}
    ...
    \caption{ \label{fig: tahi}}
    \end{subfigure}
    \caption{(a)  Known variances and $P^*=0.9$. In this example, our theorem is true: the IZ guarantee is always satisfied and the inequality is tight as $\delta$ goes to zero. (b) Unknown variances and $P^*=0.9$. In this example, our theorem is true: the IZ guarantee is satisfied and the inequality is tight as delta goes to zero. If we want that the IZ guarantee be satisfied in all the cases, we can just increment $n_{0}$. (c) Known variances, $P^*=0.9$ This was a hard example to find and it was specially chosen to illustrate our theorem. In practice, we would choose $n_{0}>0$ and the IZ guarantee will be satisfied.
\label{fig:tahi10}}
...
we can just increment the parameter $n_{0}$. Here, the points plotted have central confidence intervals of length at most $0.014$. 
...
This configurations was specially chosen to illustrate our theorem, and so it is harder than typical configurations.
...
This figure also shows that the bound is tight  on probability of correct selection. We should also note that the number of samples required increases very fast. Here, the points plotted have central confidence intervals of length at most $0.014$. 
