...
The following step is to find the limit of $q_{tx}^{F,\delta}\left(A\right)$
...
2, which allows us to use Theorem 5.5 of \citeN{billingsley:convergence}, which
...
of $\mathcal{C}\left(\delta,\hat{\tau}_{n}\left(\delta\right)\right)$. (The details may be found in a full version which will be submitted soon to arXiv). 
...
\mbox{lim}_{\delta\rightarrow0}\mathbb{P}\left(CS_{\delta}\right) & = & \mbox{lim}_{\delta\rightarrow0}\mathbb{P}\left(f\left(\mathcal{C}\left(\delta,t\right),\delta\right)=1\right)\\
 ...
 & \geq & P^{*}
\end{eqnarray*}
where the last inequality follows from the Theorem 2 of \citeN{Frazier:BIZ}.
