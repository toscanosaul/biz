...
%DIF DEL first_submission.tex   Thu Jul  2 23:06:39 2015
%DIF ADD wsc15paper.tex         Thu Jul  2 22:06:56 2015
%*
...
This procedure's lower bound on \DIFaddbegin \DIFadd{the }\DIFaddend worst-case probability of correct selection in the
preference zone is tight in continuous time, and almost tight in \DIFdelbegin \DIFdel{the
}\DIFdelend discrete time. In numerical experiments, the number of samples required
by BIZ is significantly smaller than that of procedures like the \DIFdelbegin \DIFdel{KN procedure of \mbox{%DIFAUXCMD
\citeN{Kim:2001}
}%DIFAUXCMD
and the }\DIFdelend $\mathcal{P}_{B}^{*}$ procedure of \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeN{Bechhofer:1968}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeN{Bechhofer:1968}
}%DIFAUXCMD
and the KN procedure of \mbox{%DIFAUXCMD
\citeN{Kim:2001}
}%DIFAUXCMD
}\DIFaddend , especially on problems with many alternatives. Unfortunately, the
proof from \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Frazier:BIZ}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeN{Frazier:BIZ}
}%DIFAUXCMD
}\DIFaddend that the BIZ procedure satisfies the IZ guarantee for the discrete-time case assumes that (1) samples are normally distributed; 
...
To simplify our analysis, we analyze a slight modification of the version of the BIZ procedure presented in \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Frazier:BIZ}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeN{Frazier:BIZ}
}%DIFAUXCMD
}\DIFaddend , which keeps a certain parameter $\lambda^2_z$ fixed rather than letting it vary as did \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Frazier:BIZ}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeN{Frazier:BIZ}
}%DIFAUXCMD
}\DIFaddend .  Numerical experiments on typical cases show little difference in performance between the version of BIZ we analyze and the version in \citeN{Frazier:BIZ}.
...
we recall the Bayes-inspired IZ (BIZ) procedure from \citeN{Frazier:BIZ}. In \DIFdelbegin \DIFdel{$\text{ß}4$}\DIFdelend \DIFaddbegin \DIFadd{$\mathsection$\ref{proof}}\DIFaddend ,
...
set and this \DIFdelbegin \DIFdel{remain }\DIFdelend \DIFaddbegin \DIFadd{remaining }\DIFaddend alternative is chosen as the best. \DIFaddbegin \DIFadd{It is shown in \mbox{%DIFAUXCMD
\citeN{Frazier:BIZ}
}%DIFAUXCMD
that the
...
\citeN{Frazier:BIZ} shows that the BIZ procedure satisfies the IZ guarantee 
...
variances is given below.  \DIFdelbegin \DIFdel{It }\DIFdelend \DIFaddbegin \DIFadd{This algorithm }\DIFaddend takes a variable number of samples
from \DIFdelbegin \DIFdel{each alternative }\DIFdelend \DIFaddbegin \DIFadd{alternative $x$ at time $t$}\DIFaddend , and $n_{tx}$ is this number \DIFaddbegin \DIFadd{(its definition may found in the algorithm given below)}\DIFaddend . 
...
stage of samples, and $100$ is the recommended value for $n_{0}$ \DIFaddbegin \DIFadd{when the variances are unknown}\DIFaddend . The paramater
...
we analyze and the version in \citeN{Frazier:BIZ}.
...
from alternative $x$ thus far\DIFaddbegin \DIFadd{, }\DIFaddend and $Z_{tx}=Y_{n_{tx},x}$ \DIFaddbegin \DIFadd{is the sum of
the samples from alternative $x$ observed by stage $t$}\DIFaddend .
...
when applied in continuous time (Theorem~1 in \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Frazier:BIZ}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeN{Frazier:BIZ}
...
We now construct a function $g(\cdot)$ that, when applied to the path of a $k$-dimensional standard Brownian motion, will be equal in distribution to the indicator of the correct selection event from the continuous-time BIZ procedure from \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Frazier:BIZ}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeN{Frazier:BIZ}
...
The random variable $g(W)$ is equal in distribution to the indicator of the event of correct selection that results from applying the continuous-time BIZ procedure from \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Frazier:BIZ}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeN{Frazier:BIZ}
...
\DIFaddbegin \label{numericalExperiments}
\DIFaddend 
