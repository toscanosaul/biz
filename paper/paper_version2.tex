...
The BIZ procedure maps $\left(Y_{n_{x}\left(t\right),x}:t\in\mathbb{N},x\in\left\{ 1,\ldots,k\right\} \right)$
to a selection decision, where $Y_{n_{x}\left(t\right),t}$ is the
sum of samples observed by time $t$. In more detail, it works by
transforming $\left(Y_{n_{x}\left(t\right),x}:A\subset\left\{ 1,\ldots,k\right\} ,x\in A\right)$
to $\left(q'_{tx}\left(A\right):t\in\mathbb{N},A\subset\left\{ 1,\ldots,k\right\} ,x\in A\right)$.
This $q'$ process is given by time process with jumps at $t\in\mathbb{N}$,
\[
q'_{tx}\left(A\right)=q'\left(\left(Z_{tx},t:x\in A\right),\delta,A\subset\left\{ 1,\ldots,k\right\} \right)=\mbox{exp}\left(\delta\beta_{t}\frac{Z_{tx}}{n_{tx}}\right)\left/\sum_{x'\in A}\mbox{exp}\left(\delta\beta_{t}\frac{Z_{tx}}{n_{tx}}\right)\right.
\]
where the variances are fixed. The same distribtuion over selection
decision will result if we apply the same same selection process to
the following time process with jumps at $t\in\left\{ 0,\delta^{2},2\delta^{2},\ldots\right\} =\delta^{2}\mathbb{N}$,
\[
...
process
\begin{eqnarray}
q_{tx}\left(A\right) & = & q\left(\left(Z_{\frac{t}{\delta^{2}}x},\frac{t}{\delta^{2}}:x\in A\right),\delta,A\subset\left\{ 1,\ldots,k\right\} \right)\label{eq:0.001}
\end{eqnarray}
where $t\geq0$ . Later we will show the difference in selection decisions
vanishes as $\delta\rightarrow0$.

After that, we would like to use a kind of central limit theorem for
$Z_{\frac{t}{\delta^{2}}x}=Y_{n_{x}\left(t/\delta^{2}\right),x}$
when $\delta$ goes to zero to work with a Brownian motion instead.
Specifically, we are going to see that the the standardized sum of
the output data converges to a Brownian motion in the sense of $D_{\infty}:=D[0,\infty)$,
...
found in Chapter 3 of Billingsley 1999 and the appendix. We will first
see this result because it will give us a clue for proving the asymptotic
validity of the Algorithm 2.
...
We now gain the insight that we were looking for. We will use $\mathcal{C}_{x}\left(\delta,\cdot\right)$
instead of $Z_{\frac{t}{\delta^{2}}x}$ in $\left(1\right)$, which
is the definition of $q_{tx}\left(A\right)$,
\[
...
\]
the previous equation is true beucase 
\[
Z_{\frac{t}{\delta^{2}}x}=C_{x}\left(\delta,t\right)\frac{\lambda_{x}^{2}}{\delta\lambda_{z}^{2}}+\frac{\lambda_{x}^{2}}{\lambda_{z}^{2}}\left(n_{0}+\frac{t}{\delta^{2}}\right).
\]
...
In order to prove this, we first prove Lemma 2, which will allow us
to use Theorem 5.5 of Billingsley 1968, which implies the desired
result.
...
Skorohod convergente, then $\mbox{min}\left(f_{n},g_{n}\right)\rightarrow\mbox{min}\left(f,g\right)$
...
\]
Let $\epsilon$ be the quantity on the righ-hand side of this inequality,
...
new sequential elemination procedure. This algorithm is relevant because
...
defining a metric on the space. The Skorohod metricric $d_{t}$ on
$D\left[0,t\right]$ for each $t\geq0$ is:
\[
